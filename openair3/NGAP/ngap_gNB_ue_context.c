...
void ngap_gNB_allocate_new_UE_context(struct ngap_gNB_ue_context_s *ue_desc_p)
{
...
struct ngap_gNB_ue_context_s *ngap_gNB_get_ue_context(uint32_t gNB_ue_ngap_id)
{
  ngap_gNB_ue_context_t temp = {.gNB_ue_ngap_id = gNB_ue_ngap_id};
  return RB_FIND(ngap_ue_map, &ngap_ue_head, &temp);
}

void ngap_gNB_free_ue_context(uint32_t gNB_ue_ngap_id)
{
  struct ngap_gNB_ue_context_s *tmp = ngap_gNB_get_ue_context(gNB_ue_ngap_id);
  if (tmp == NULL) {
