    ...
    if (num_gps_locked == num_mboards and num_mboards > 1) {
      ...
      for (size_t mboard = 1; mboard < num_mboards; mboard++) {
        ...
        if (mboard_time != mboard0_time) {
          all_matched = false;
          std::cerr << (boost::format("ERROR: Times are not aligned: USRP 0=%0.9f, USRP %d=%0.9f") % mboard0_time.get_real_secs() % mboard % mboard_time.get_real_secs()) << std::endl;
        }
      }
...
static int trx_usrp_read(openair0_device *device, openair0_timestamp *ptimestamp, void **buff, int nsamps, int cc)
{
  ...
  *ptimestamp = s->rx_timestamp;
  ...
  if (recPlay != NULL) { // record mode
    ...
    if (recPlay->nbSamplesBlocks <= device->openair0_cfg->recplay_conf->u_sf_max && recPlay->maxSizeBytes >= (recPlay->currentPtr - (uint8_t *)recPlay->ms_sample) + sizeof(iqrec_t) + nsamps * 4) {
...
void *freq_thread(void *arg)
{
  openair0_device *device = (openair0_device *)arg;
  usrp_state_t *s = (usrp_state_t *)device->priv;
  uhd::tune_request_t tx_tune_req(device->openair0_cfg[0].tx_freq[0], device->openair0_cfg[0].tune_offset);
  uhd::tune_request_t rx_tune_req(device->openair0_cfg[0].rx_freq[0], device->openair0_cfg[0].tune_offset);
 ...
 * \returns 0 in success
 */
int trx_usrp_set_gains(openair0_device *device, openair0_config_t *openair0_cfg)
{
  usrp_state_t *s = (usrp_state_t *)device->priv;
...
rx_gain_calib_table_t calib_table_b210[] = {{3500000000.0, 44.0}, {2660000000.0, 49.0}, {2300000000.0, 50.0}, {1880000000.0, 53.0}, {816000000.0, 58.0}, {-1, 0}};

/*! \brief USRPB210 RX calibration table */
rx_gain_calib_table_t calib_table_b210_38[] = {{3500000000.0, 44.0}, {2660000000.0, 49.8}, {2300000000.0, 51.0}, {1880000000.0, 53.0}, {816000000.0, 57.0}, {-1, 0}};

/*! \brief USRPx310 RX calibration table */
rx_gain_calib_table_t calib_table_x310[] = {{3500000000.0, 77.0}, {2660000000.0, 81.0}, {2300000000.0, 81.0}, {1880000000.0, 82.0}, {816000000.0, 85.0}, {-1, 0}};

/*! \brief USRPn3xf RX calibration table */
rx_gain_calib_table_t calib_table_n310[] = {{3500000000.0, 0.0}, {2660000000.0, 0.0}, {2300000000.0, 0.0}, {1880000000.0, 0.0}, {816000000.0, 0.0}, {-1, 0}};

/*! \brief Empty RX calibration table */
rx_gain_calib_table_t calib_table_none[] = {{3500000000.0, 0.0}, {2660000000.0, 0.0}, {2300000000.0, 0.0}, {1880000000.0, 0.0}, {816000000.0, 0.0}, {-1, 0}};
...
void set_rx_gain_offset(openair0_config_t *openair0_cfg, int chain_index, int bw_gain_adjust)
{
  ...
  while (openair0_cfg->rx_gain_calib_table[i].freq > 0) {
    ...
    LOG_I(HW, "cal %d: freq %f, offset %f, diff %f\n", i, openair0_cfg->rx_gain_calib_table[i].freq, openair0_cfg->rx_gain_calib_table[i].offset, diff);
...
int device_init(openair0_device *device, openair0_config_t *openair0_cfg)
{
  ...
  for (int i = 0; i < ((int)s->usrp->get_rx_num_channels()); i++) {
    if (i < openair0_cfg[0].rx_num_channels) {
      ...
      uhd::tune_request_t rx_tune_req(openair0_cfg[0].rx_freq[i], openair0_cfg[0].tune_offset);
  ...
  for (int i = 0; i < ((int)s->usrp->get_tx_num_channels()); i++) {
    ...
    if (i < openair0_cfg[0].tx_num_channels) {
      ...
      uhd::tune_request_t tx_tune_req(openair0_cfg[0].tx_freq[i], openair0_cfg[0].tune_offset);
  ...
  if (recPlay != NULL) { // record mode
    recPlay->maxSizeBytes = openair0_cfg[0].recplay_conf->u_sf_max * (sizeof(iqrec_t) + BELL_LABS_IQ_BYTES_PER_SF);
