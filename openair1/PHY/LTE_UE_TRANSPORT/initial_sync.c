...
int pbch_detection(PHY_VARS_UE *ue, runmode_t mode) {
  ...
  LOG_X(PHY,"[UE%d] Initial sync: starting PBCH detection (rx_offset %d)\n",ue->Mod_id,
        ue->rx_offset);
  ...
  LOG_X(PHY,"[UE %d] RX RSSI %d dBm, digital (%d, %d) dB, linear (%d, %d), avg rx power %d dB (%d lin), RX gain %d dB\n",
        ue->Mod_id,
        ...
        ue->rx_total_gain_dB);
  LOG_X(PHY,"[UE %d] N0 %d dBm digital (%d, %d) dB, linear (%d, %d), avg noise power %d dB (%d lin)\n",
        ue->Mod_id,
  ...
  if (pbch_decoded) {
    ...
    if(ue->FeMBMS_active != 2) {
      ...
      LOG_X(PHY,"[UE%d] Initial sync: pbch decoded sucessfully p %d, tx_ant %d, frame %d, N_RB_DL %d, phich_duration %d, phich_resource %s!\n",
            ue->Mod_id,
      ...
      LOG_X(PHY,"[UE%d] Initial sync: FeMBMS pbch decoded sucessfully p %d, tx_ant %d, frame %d, N_RB_DL %d, AdditionalNonMBSFN_SF %d, frame_mod4 %d\n",
            ue->Mod_id,
            frame_parms->nb_antenna_ports_eNB,
            pbch_tx_ant,
            ue->proc.proc_rxtx[0].frame_rx,
...
int initial_sync(PHY_VARS_UE *ue, runmode_t mode) {
  ...
  */
  ...
  LOG_X(PHY,"[UE%d] Initial sync : Estimated PSS position %d, Nid2 %d\n",ue->Mod_id,sync_pos,ue->common_vars.eNb_id);
  // SSS detection
  ...
  if (((sync_pos2 - sync_pos_slot) >=0 ) &&
    ...
    LOG_X(PHY,"Calling sss detection (FDD normal CP)\n");
    ...
    LOG_X(PHY,"FDD Normal prefix: CellId %d metric %d, phase %d, flip %d, pbch %d\n",
    ...
    if (ret==-1) {
      ...
      LOG_X(PHY,"FeMBMS Normal prefix: CellId %d metric %d, phase %d, flip %d, pbch %d\n",
            frame_parms->Nid_cell,metric_fdd_ncp,phase_fdd_ncp,flip_fdd_ncp,ret);
    }
  } else {
    LOG_X(PHY,"FDD Normal prefix: SSS error condition: sync_pos %d, sync_pos_slot %d\n", sync_pos, sync_pos_slot);
  }
  ...
  if (ret==-1) {
    ...
    if (((sync_pos2 - sync_pos_slot) >=0 ) &&
      ...
      LOG_X(PHY,"FDD Extended prefix: CellId %d metric %d, phase %d, flip %d, pbch %d\n",
      ...
      LOG_X(PHY,"FDD Extended prefix: SSS error condition: sync_pos %d, sync_pos_slot %d\n", sync_pos, sync_pos_slot);
    }
    ...
    if (ret==-1) {
      ...
      LOG_X(PHY,"TDD Normal prefix: CellId %d metric %d, phase %d, flip %d, pbch %d\n",
      ...
      if (ret==-1) {
        ...
        LOG_X(PHY,"TDD Extended prefix: CellId %d metric %d, phase %d, flip %d, pbch %d\n",
  ...
  if (ret==0) {  // PBCH found so indicate sync to higher layers and configure frame parameters
    ...
    LOG_X(PHY,"[UE%d] Initial sync : PBCH not ok\n",ue->Mod_id);
    LOG_X(PHY,"[UE%d] Initial sync : Estimated PSS position %d, Nid2 %d\n",ue->Mod_id,sync_pos,ue->common_vars.eNb_id);
    ...
    LOG_X(PHY,"[UE%d] Initial sync : Estimated Nid_cell %d, Frame_type %d\n",ue->Mod_id,
