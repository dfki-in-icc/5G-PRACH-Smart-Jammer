...
void sldch_decoding(PHY_VARS_UE *ue,UE_rxtx_proc_t *proc,int frame_rx,int subframe_rx,int npsdch,int nprb,int rvidx) {
  ...
  LOG_M("sldch_rxF_ext.m","sldchrxF_ext",rxdataF_ext[0],14*12*ue->frame_parms.N_RB_DL,1,1);
#endif
  
  lte_ul_channel_estimation_RRU(&ue->frame_parms,
...
#ifdef PSDCH_DEBUG
  LOG_M("drs_ext0.m","drsest0",drs_ch_estimates[0],ue->frame_parms.N_RB_UL*12*14,1,1);
#endif
...
#ifdef PSDCH_DEBUG
  LOG_M("sldch_rxF_comp.m","sldchrxF_comp",rxdataF_comp[0],ue->frame_parms.N_RB_UL*12*14,1,1);
#endif
...
#ifdef PSDCH_DEBUG  
  LOG_M("sldch_llr.m","sldchllr",ue->sldch_ulsch_llr[ue->current_thread_id[subframe_rx]],
 ...
 //     	       ((r*Cmux)+i)<<1,ue->sldch_dlsch_llr[((r*Cmux)+i)<<1],j-2,(((r*Cmux)+i)<<1)+1,ue->sldch_dlsch_llr[(((r*Cmux)+i)<<1)+1],j-1);
  ...
  //printf("ue->dlsch_rx_sldch[npsdch]->max_turbo_iterations %d ret %d\n",ue->dlsch_rx_sldch[npsdch]->max_turbo_iterations,ret);
...
void sldch_codingmodulation(PHY_VARS_UE *ue,int frame_tx,int subframe_tx,int nprb,int rvidx) {
  ...
  //for (int i=0;i<2*12*2*((Nsymb-1)<<1)/16;i++) printf("encoding: E[%d] %d\n",i,dlsch->harq_processes[0]->eDL[i]);
  int Cmux = (Nsymb-1)<<1;
  uint8_t *eptr;
  for (int i=0,j=0; i<Cmux; i++)
    ...
    for (int r=0; r<24; r++) {
       if (dlsch->harq_processes[0]->Qm == 2) {
           eptr=&dlsch->harq_processes[0]->eDL[((r*Cmux)+i)<<1];
           ...
           eptr=&dlsch->harq_processes[0]->eDL[((r*Cmux)+i)<<2];
           ulsch->h[j++] = *eptr++;
           ulsch->h[j++] = *eptr++;  
           ulsch->h[j++] = *eptr++;      
           ulsch->h[j++] = *eptr++;        
  ...
  if (ue->generate_ul_signal_LTE[subframe_tx][0] == 0) 
  ...
  ue->generate_ul_signal_LTE[subframe_tx][0] = 1;
...
void check_and_generate_psdch(PHY_VARS_UE *ue,int frame_tx,int subframe_tx) {
  ...
  //printf("psdch_generated\n");
  sldch->j++;
  sldch->j&=3;
  if (sldch->j==0) ue->sldch_active=0;
  //printf("sldch_active=%d\n",ue->sldch_active);
}
