    ...
    for (int l=start_symbol; l<start_symbol+number_of_symbols; l++) {
      ...
      for (i=0; i< nb_rb*NR_NB_SC_PER_RB; i++) {
        ...
        if (is_dmrs == 1) {
          ...
          if (pusch_pdu->transform_precoding == transformPrecoder_disabled) {
            ((int16_t*)tx_precoding[nl])[(sample_offsetF)<<1]       = ((int16_t *)tx_layers[nl])[m<<1]*beta_ulsch;
            ((int16_t*)tx_precoding[nl])[((sample_offsetF)<<1) + 1] = ((int16_t *)tx_layers[nl])[(m<<1) + 1]*beta_ulsch;
            ...
            ((int16_t*)tx_precoding[nl])[(sample_offsetF)<<1]       = ((int16_t *) y)[m<<1]*beta_ulsch;
            ((int16_t*)tx_precoding[nl])[((sample_offsetF)<<1) + 1] = ((int16_t *) y)[(m<<1) + 1]*beta_ulsch;
