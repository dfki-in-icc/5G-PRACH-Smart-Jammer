...
int32_t generate_nr_prach(PHY_VARS_NR_UE *ue, uint8_t gNB_id, uint8_t slot){
  ...
  *************************************************************************/
  ...
  LOG_I(PHY, "PRACH [UE %d] in slot %d, placing PRACH in position %d, msg1 frequency start %d (k1 %d), preamble_offset %d, first_nonzero_root_idx %d RACH format %d\n", 
        Mod_id,
        slot,
        k,
        n_ra_prb,
  ...
  #ifdef NR_PRACH_DEBUG
    LOG_X(PHY, "PRACH [UE %d] Ncp %d, dftlen %d \n", Mod_id, Ncp, dftlen);
  #endif
