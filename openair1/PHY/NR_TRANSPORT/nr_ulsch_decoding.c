...
/*! \file PHY/NR_TRANSPORT/nr_ulsch_decoding.c
* \brief (Fixed .so issue)FPGA accelerator integrated into OAI (for one and multi code block)
* \author Sendren Xu, SY Yeh(fdragon), Hongming, Terng-Yin Hsu, Chao-Hong Hsu
* \date 2022-08-23
* \version 6.0
...
// PUYOMI
// #include "PHY/CODING/nrLDPC_decoder/ldpc_decoder_pym.h"
#define PRINT_CRC_CHECK 1
#define UL_LDPC_MODE 1
...
void clean_gNB_ulsch(NR_gNB_ULSCH_t *ulsch)
{
  ...
  if (ulsch)
  {
    ...
    for (i = 0; i < NR_MAX_SLOTS_PER_FRAME; i++)
      ulsch->harq_process_id[i] = 0;
    ...
    for (i = 0; i < NR_MAX_ULSCH_HARQ_PROCESSES; i++)
    {
      if (ulsch->harq_processes[i])
      {
        ...
        for (j = 0; j < MAX_CQI_BYTES; j++)
          ulsch->harq_processes[i]->o[j] = 0;
        ...
        for (j = 0; j < MAX_ACK_PAYLOAD; j++)
          ulsch->harq_processes[i]->q_ACK[j] = 0;
        for (j = 0; j < MAX_RI_PAYLOAD; j++)
          ulsch->harq_processes[i]->q_RI[j] = 0;
...
char multi_indata[27000 * 25];
char multi_outdata[1100 * 25];

uint32_t G_pym;
uint8_t n_layers_pym;
...
void nr_processULSegment(void *arg)
{
  ...
  uint32_t G_pym2 = G_pym;
  uint8_t n_layers_pym2 = n_layers_pym;
  r_offset = 0;
  for (r = 0; r < ulsch_harq->C; r++)
  {
    ...
    E = nr_get_E(G_pym2, ulsch_harq->C, Qm, n_layers_pym2, r);
    ...
    for (int ab = 0; ab < input_CBoffset; ab++)
    {
      ...
      if (l[ab] == -128)
      {
        multi_indata[ab + r * input_CBoffset] = 0x81;
      }
      else
      {
        multi_indata[ab + r * input_CBoffset] = ((int8_t)l[ab]);
      }

      multi_indata[ab + r * input_CBoffset] = ((char)multi_indata[ab + r * input_CBoffset] ^ 0xFF) + 1;
    }
    clock_gettime(CLOCK_MONOTONIC, &end2);
    // printf("total_time %.2f usec\n", (end2.tv_nsec - start2.tv_nsec) *1.0 / 1000);
    total_time = total_time + ((end2.tv_nsec - start2.tv_nsec) * 1.0 / 1000);

    if (phy_vars_gNB->fpga_enabled == 0)
    {
      start_meas(&phy_vars_gNB->ulsch_ldpc_decoding_stats);
      no_iteration_ldpc = nrLDPC_decoder(p_decoderParms,
                                         (int8_t *)&pl[0],                // Input
                                         llrProcBuf,                      // Output
                                         ulsch_harq->p_nrLDPC_procBuf[r], // Output
                                         p_procTime);
      stop_meas(&phy_vars_gNB->ulsch_ldpc_decoding_stats);
      /////////////////////////////////////LLR_Output /////////////////////////////////////
      // printf(GREEN "\n------------------------------------[llrProcBuf (LLR Output)]: ------------------------------------\n" CLOSE);
      // int count_dec = 0;

      // for (int g = 0; g <= 15; g++)
      // {
      //   printf(" %d,", (uint8_t)llrProcBuf[g]);
      // }
      // printf(GREEN "\n------------------------------------------------------------------------------------------------\n" CLOSE);
#ifdef PRINT_CRC_CHECK
      printf("[CRC Check]\n");
      if (check_crc((uint8_t *)llrProcBuf, length_dec, ulsch_harq->F, crc_type))
      {
        if (crc_type == 0)
        {
          printf("*******************crc type: CRC 24_A *********************\n");
        }
        else if (crc_type == 1)
        {
          printf("*******************crc type: CRC 24_B *********************\n");
        }
        else if (crc_type == 2)
        {
          printf("*******************crc type: CRC 16 *********************\n");
        }
        else
        {
          printf("*******************crc type: CRC 8 *********************\n");
        }
#ifdef PRINT_CRC_CHECK
        LOG_I(PHY, L_GREEN "Segment %d CRC OK, iterations %d/%d\n\n" CLOSE, r, no_iteration_ldpc, max_ldpc_iterations);
#endif
      ...
      for (int m = 0; m < Kr >> 3; m++) // Kr=K=22Zc or 10Zc = Message bits length
      {
...
#endif // PRINT_CRC_CHECK

  if (phy_vars_gNB->fpga_enabled == 1) /* FPGA VISION */
  {
...
//=======================================================================================================================
      ...
      if (check_crc((uint8_t *)llrProcBuf, length_dec, ulsch_harq->F, crc_type))
      {
        ...
        printf("FPGA LDPC [%d/%d] CRC check is OK \n", i + 1, dec_conf.numCB);
      }
#endif
                           ...
                           uint32_t G)
{
  ...
  // total_time
  printf(L_PURPLE "FPGA_CB time:%.2f usec\n" CLOSE, (end1.tv_nsec - start1.tv_nsec) * 1.0 / 1000);
  printf("total:%.2f usec\n", (ts_end.tv_nsec - ts_start.tv_nsec) * 1.0 / 1000);
  // printf("%.2f\n", total_time);
  // printf("%.2f\n", (end1.tv_nsec - start1.tv_nsec) *1.0 / 1000);
  // printf("%.2f\n", (ts_end.tv_nsec - ts_start.tv_nsec) *1.0 / 1000);
  //////////////////////////////////////////////////////////////////////////////////////////
  // }
  return 1;
