...
#ifdef DEBUG_RAR
  LOG_X(PHY,"ulsch ra (eNB): harq_pid %d\n",harq_pid);
  LOG_X(PHY,"ulsch ra (eNB): NBRB     %d\n",ulsch_harq->nb_rb);
  LOG_X(PHY,"ulsch ra (eNB): rballoc  %x\n",ulsch_harq->first_rb);
  LOG_X(PHY,"ulsch ra (eNB): harq_pid %d\n",harq_pid);
  LOG_X(PHY,"ulsch ra (eNB): round    %d\n",ulsch_harq->round);
  LOG_X(PHY,"ulsch ra (eNB): TBS      %d\n",ulsch_harq->TBS);
  LOG_X(PHY,"ulsch ra (eNB): mcs      %d\n",ulsch_harq->Msc_initial);
  LOG_X(PHY,"ulsch ra (eNB): Or1      %d\n",ulsch_harq->Or1);
  LOG_X(PHY,"ulsch ra (eNB): ORI      %d\n",ulsch_harq->O_RI);
#endif
  return(0);
}
