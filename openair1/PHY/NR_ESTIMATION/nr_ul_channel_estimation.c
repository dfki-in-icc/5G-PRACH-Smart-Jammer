...
int nr_pusch_channel_estimation(PHY_VARS_gNB *gNB,
  ...
  for (aarx=0; aarx<gNB->frame_parms.nb_antennas_rx; aarx++) {
    ...
    if (pusch_pdu->dmrs_config_type == pusch_dmrs_type1 && chest_freq == 0){
      ...
      for (int n = 0; n < 3*nb_rb_pusch; n++) {
        ...
        for (int k_line = 0; k_line <= 1; k_line++) {
          re_offset = (k + (n << 2) + (k_line << 1) + delta) % gNB->frame_parms.ofdm_symbol_size;
        ...
        for (int k_line = 0; k_line <= 1; k_line++) {
#ifdef DEBUG_PUSCH
          re_offset = (k + (n << 2) + (k_line << 1)) % gNB->frame_parms.ofdm_symbol_size;
          rxF = (int16_t *) &rxdataF[aarx][(soffset + symbol_offset + re_offset)];
	...
	rxF += 24;
      ...
      else rxF = (int16_t*)&rxdataF[aarx][(symbol_offset+nushift)];
