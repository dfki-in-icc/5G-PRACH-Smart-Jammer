...
int phy_procedures_nrUE_RX(PHY_VARS_NR_UE *ue,
  ...
  for (uint16_t l=0; l<14; l++) {
    for(int n_ss = 0; phy_pdcch_config->nb_search_space; n_ss++) {
      if((phy_pdcch_config->pdcch_config[n_ss].coreset.StartSymbolIndex <= l)&&((phy_pdcch_config->pdcch_config[n_ss].coreset.StartSymbolIndex+phy_pdcch_config->pdcch_config[n_ss].coreset.duration > l))){
        temp_cnt++;
        break;
      }
    }
  }
  uint8_t nb_symb_pdcch=temp_cnt;
#ifdef NR_PDCCH_SCHED
  ...
  for(int n_ss = 0; n_ss<phy_pdcch_config->nb_search_space; n_ss++) {
    ...
    for (uint16_t l=0; l<nb_symb_pdcch; l++) {
      if((phy_pdcch_config->pdcch_config[n_ss].coreset.StartSymbolIndex <= l)&&((phy_pdcch_config->pdcch_config[n_ss].coreset.StartSymbolIndex+phy_pdcch_config->pdcch_config[n_ss].coreset.duration > l))){
        int32_t *temp_IQ=&ue->common_vars.rxdataF[0][ue->frame_parms.ofdm_symbol_size*l];
        ...
        LOG_D(PHY, "pdcch_channel_estimation: sf %d slot %d sym %d first_carrier_offset %d, BWPStart %d, coreset_start_rb %d %d power %lf\n",
          frame_rx%1024, nr_slot_rx,l,fp->first_carrier_offset, phy_pdcch_config->pdcch_config[n_ss].BWPStart, coreset_start_rb, coreset_nb_rb,temp_power_db);
