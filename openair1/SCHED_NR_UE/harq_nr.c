...
void config_uplink_harq_process(PHY_VARS_NR_UE *ue, int gNB_id, int code_word_idx, uint8_t number_harq_processes_pusch, nr_ue_phy_vars_data_t *phy_vars)
{
  ...
  for (int slot_tx = 0; slot_tx < NR_MAX_SLOTS_PER_FRAME; slot_tx++) {
    phy_vars->ulsch[gNB_id]->harq_process_id[slot_tx] = NR_MAX_HARQ_PROCESSES;
  }
}
...
{
  NR_UE_ULSCH_t *ulsch = phy_vars->ulsch[gNB_id];
