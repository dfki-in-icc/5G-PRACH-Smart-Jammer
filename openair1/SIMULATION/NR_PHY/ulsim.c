...
int rrc_gNB_process_GTPV1U_CREATE_TUNNEL_RESP(
    const protocol_ctxt_t *const ctxt_pP,
...
int nr_rrc_gNB_process_GTPV1U_CREATE_TUNNEL_RESP(
    const protocol_ctxt_t *const ctxt_pP,
...
int main(int argc, char **argv)
{
  ...
  while ((c = getopt(argc, argv, "a:b:c:d:ef:g:h:ikl:m:n:op:r:s:u:w:y:z:F:G:H:M:N:PR:S:T:U:L:Z")) != -1)
  {
  ...
  break;
    ...
    case 'h':
      ...
      printf("-o Enables FPGA LDPC decoder function (default = 0)\n");
  ...
  if (N_RB_UL >= 217)
    ...
    sampling_frequency = 32.72;
  else
  {
    printf("Need at least 106 PRBs\b");
    exit(-1);
  }
  if (N_RB_UL == 273)
    bandwidth = 100;
  ...
  if (openair0_cfg[0].threequarter_fs == 1)
  ...
  */
  ...
  for (SNR = snr0; SNR < snr1; SNR += snr_step)
  {
    ...
    for (trial = 0; trial < n_trials; trial++)
    {
      ...
      while (round < max_rounds && crc_status)
      {
        ...
        for (int i = 0; i < (TBS / 8); i++)
          ulsch_ue[0]->harq_processes[harq_pid]->a[i] = i & 0xff;
        if (input_fd == NULL)
        {
      ...
      if (n_trials == 1 && errors_scrambling[0][snrRun] > 0)
      {
        printf("\x1B[31m"
               ...
               "\x1B[0m",
               frame, trial, errors_scrambling[0][snrRun]);
      }
      ...
      if (errors_decoding > 0 && error_flag == 0)
      {
        ...
        if (n_trials == 1)
          printf("\x1B[31m"
                 ...
                 "\x1B[0m",
                 frame, trial, errors_decoding);
      }
      roundStats[snrRun] += ((float)round);
      if (!crc_status)
        effRate[snrRun] += ((double)TBS) / (double)round;
    } // trial loop
    ...
    if (print_perf == 1)
    {
      ...
      printStatIndent2(&gNB->ulsch_ldpc_fpga_time_stats, "ULSCH FPGA LDPC decoding");
      printf("\n");
    }
