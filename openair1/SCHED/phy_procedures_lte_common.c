...
unsigned char ul_ACK_subframe2_dl_subframe(LTE_DL_FRAME_PARMS *frame_parms,unsigned char subframe,unsigned char ACK_index)
{
  ...
  if (frame_parms->frame_type == FDD) {
    ...
    case 1:
      ...
      } else AssertFatal(1==0,"illegal subframe %d for tdd_config %d\n",
                         subframe,frame_parms->tdd_config);
      break;
    case 2:
      if (subframe==2 && ACK_index<3) return(4+ACK_index);
      else if (subframe == 2) return(8);
      else if (subframe == 7 && ACK_index<3) return((9+ACK_index)%10);
      else if (subframe == 7) return(3);
      else AssertFatal(1==0,"illegal subframe %d for tdd_config %d\n",
                       subframe,frame_parms->tdd_config);
      break;
    } 
...
int ul_ACK_subframe2_dl_frame(LTE_DL_FRAME_PARMS *frame_parms,int frame, unsigned char subframe,unsigned char subframe_tx)
{
  ...
  if (frame_parms->frame_type == FDD) {
    ...
    case 1:
      ...
      break;
    case 2:
      AssertFatal(subframe==2 || subframe==7,"illegal UL subframe for for TDD Config 2\n");
      return(4);
      break;
    case 3:
        //TODO
