...
void init_transport(PHY_VARS_eNB *eNB)
{
  ...
  for (int i = 0; i < NUMBER_OF_DLSCH_MAX; i++) {
    ...
    for (int j = 0; j < 2; j++) {
      AssertFatal((eNB->dlsch[i][j] = new_eNB_dlsch(1, 8, NSOFT, fp->N_RB_DL, 0, fp)) != NULL, "Can't get eNB dlsch structures for UE %d \n", i);
  ...
  for (int i = 0; i < NUMBER_OF_ULSCH_MAX; i++) {
    ...
    AssertFatal((eNB->ulsch[1 + i] = new_eNB_ulsch(MAX_TURBO_ITERATIONS, fp->N_RB_UL, 0)) != NULL, "Can't get eNB ulsch structures\n");
  ...
  // ULSCH for RA
  AssertFatal((eNB->ulsch[0] = new_eNB_ulsch(MAX_TURBO_ITERATIONS, fp->N_RB_UL, 0)) != NULL, "Can't get eNB ulsch structures\n");
  ...
  for (int inst = 0; inst < RC.nb_inst; inst++) {
    for (int CC_id = 0; CC_id < RC.nb_CC[inst]; CC_id++) {
      ...
      for (int ru_id = 0, aa = 0; ru_id < eNB->num_RU; ru_id++) {
        ...
        AssertFatal(eNB->RU_list[ru_id]->common.rxdataF != NULL, "RU %d : common.rxdataF is NULL\n", eNB->RU_list[ru_id]->idx);
        AssertFatal(eNB->RU_list[ru_id]->prach_rxsigF != NULL, "RU %d : prach_rxsigF is NULL\n", eNB->RU_list[ru_id]->idx);
                  ...
                  uint8_t br_flag)
{
  ...
  if (br_flag == 0) {
    rx_prach0(eNB, ru, proc->frame_prach, proc->subframe_prach, max_preamble, max_preamble_energy, max_preamble_delay, avg_preamble_energy, Nf, tdd_mapindex, 0, 0);
    ...
    for (i = 0; i < 4; i++) {
      if ((eNB->frame_parms.prach_emtc_config_common.prach_ConfigInfo.prach_CElevel_enable[i] == 1) && ((prach_mask & (1 << (i + 1))) > 0)) { // check that prach CE level is active now
        ...
        rx_prach0(eNB, ru, proc->frame_prach, proc->subframe_prach_br, max_preamble, max_preamble_energy, max_preamble_delay, avg_preamble_energy, Nf, tdd_mapindex, 1, i);
...
void prach_procedures_ocp(PHY_VARS_eNB *eNB, L1_rxtx_proc_t *proc, int br_flag)
{
  ...
  ocp_rx_prach(eNB, proc, eNB->RU_list[0], &max_preamble[0], &max_preamble_energy[0], &max_preamble_delay[0], &avg_preamble_energy[0], proc->frame_prach, 0, br_flag);
  LOG_D(PHY, "RACH detection index 0: max preamble: %u, energy: %u, delay: %u, avg energy: %u\n", max_preamble[0], max_preamble_energy[0], max_preamble_delay[0], avg_preamble_energy[0]);
  ...
  } else if ((eNB->prach_energy_counter == 100) && (max_preamble_energy[0] > eNB->measurements.prach_I0 + eNB->prach_DTX_threshold)) {
    LOG_I(PHY,
...
void ocp_tx_rf(RU_t *ru, L1_rxtx_proc_t *proc)
{
  ...
  if ((SF_type == SF_DL) || (SF_type == SF_S)) {
    ...
    if (SF_type == SF_S) {
       ...
       * the default.
       */
      siglen = (fp->ofdm_symbol_size + fp->nb_prefix_samples0) + (fp->dl_symbols_in_S_subframe - 1) * (fp->ofdm_symbol_size + fp->nb_prefix_samples) + ru->end_of_burst_delay;
      flags = 3; // end of burst
    }

    if (fp->frame_type == TDD && SF_type == SF_DL && prevSF_type == SF_UL) {
    ...
    ru->rfdevice.trx_write_func(&ru->rfdevice, proc->timestamp_tx + ru->ts_offset - ru->openair0_cfg.tx_sample_advance - sf_extension, txp, siglen + sf_extension, ru->nb_tx, flags);
    LOG_D(PHY, "[TXPATH] RU %d tx_rf, writing to TS %llu, frame %d, subframe %d\n", ru->idx, (long long unsigned int)proc->timestamp_tx, proc->frame_tx, proc->subframe_tx);
  }
...
#define PACKAGE_VERSION "UNKNOWN-EXPERIMENTAL"
  ...
  if (RC.nb_RU > 0 && NFAPI_MODE != NFAPI_MODE_VNF) {
    ...
    ocp_init_RU(&ru, get_softmodem_params()->rf_config_file, get_softmodem_params()->send_dmrs_sync);
    ru.rf_map.card = 0;
