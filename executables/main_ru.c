...
#define PACKAGE_VERSION "UNKNOWN-EXPERIMENTAL"
  ...
  pthread_mutex_init(&ru_mutex, NULL);
  pthread_cond_t ru_cond;
  pthread_cond_init(&ru_cond, NULL);
  ...
  if (strcmp(*(RUParamList.paramarray[j][RU_LOCAL_RF_IDX].strptr), "yes") == 0) {
    if (!(config_isparamset(RUParamList.paramarray[j], RU_LOCAL_IF_NAME_IDX))) {
      ...
      if (strcmp(*(RUParamList.paramarray[j][RU_OTA_SYNC_ENABLE_IDX].strptr), "yes") == 0)
        ru->ota_sync_enable = 1;
      else
        ru->ota_sync_enable = 0;
    }
    ...
    for (int i = 0; i < ru->num_bands; i++)
      ru->band[i] = RUParamList.paramarray[j][RU_BAND_LIST_IDX].iptr[i];
