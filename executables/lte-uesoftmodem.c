...
#define PACKAGE_VERSION "UNKNOWN-EXPERIMENTAL"
  ...
  if (NFAPI_MODE == NFAPI_UE_STUB_PNF) { // UE-STUB-PNF
    ...
    init_UE(NB_UE_INST, eMBMS_active, uecap_xer_in, 0, get_softmodem_params()->phy_test, UE_scan, UE_scan_carrier, mode, (int)rx_gain[0][0], tx_max_power[0], frame_parms[0]);
  }
  ...
  if (input_fd) {
    ...
    if (fread(PHY_vars_UE_g[0][0]->common_vars.rxdata[0], sizeof(int32_t), frame_parms[0]->samples_per_tti * 10, input_fd) != frame_parms[0]->samples_per_tti * 10)
