            ...
            for(uint8_t ant_id = 0; ant_id < xran_max_antenna_nr; ant_id++)
            {
                for(int32_t sym_idx = 0; sym_idx < XRAN_NUM_OF_SYMBOL_PER_SLOT; sym_idx++) 
                {
                    ...
                    if(ptr && pos)
                    {
                        ...
                        {
                            struct xran_section_desc *sec_desc = NULL;
                            p_prbMapElm = &pRbMap->prbMap[idxElm];
                            sec_desc =  p_prbMapElm->sec_desc[sym_id];

                            if(sec_desc == NULL)
                            {
                                printf ("sec_desc == NULL\n");
                                exit(-1);
                            }
                            ...
                            sec_desc->iq_buffer_offset = RTE_PTR_DIFF(dst, u8dptr);
                            sec_desc->iq_buffer_len = payload_len;
...
int compute_xran_statistics(void *xranlib_)
{
    ...
    {
        xran_get_time_stats(&nTotalTime, &nUsedTime, &nNumCoreUsed, &nCoreUsed, 1);
...
int read_prach_data(ru_info_t *ru, int frame, int slot)
{
    ...
    /* MP_2022: retrieve how to know XRAN port ID value */
    void *xran_device_ctx = NULL;
    uint32_t portId = (uint32_t) xran_dev_ctx_get_port_id(xran_device_ctx);

    uint32_t subframe = XranGetSubFrameNum(tti, 2, 10);
    uint32_t is_prach_slot = xran_is_prach_slot(portId, subframe, (slot % 2));
    int sym_idx = 0;
