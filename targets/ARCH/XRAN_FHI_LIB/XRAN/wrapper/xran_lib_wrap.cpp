...
json read_json_from_file_wrap(const std::string &filename)
{
    json result;
...
// Sofia: define the sizes according to the specified parameters in the conf file not up to the maximim boudary
    ...
    {
        status = xran_sector_get_instances(0, m_xranhandle, nInstanceNum, &m_nInstanceHandle[k][0]);
        if(status != XRAN_STATUS_SUCCESS) 
        {
            ...
            for(z = 0; z < xran_max_antenna_nr; z++) 
            {
                ...
                for (elm_id = 0; elm_id < XRAN_MAX_SECTIONS_PER_SYM; elm_id++)
                {
                    ...
                    for(k = 0; k < XRAN_NUM_OF_SYMBOL_PER_SLOT; k++)
                    {
                        ...
                        /* Fix MP_2022: Refer to xran_fh_o_du.h, struct sec_desc has now two dimensions */
                        struct xran_section_desc *p_loc_sec_desc = NULL;
                        p_loc_sec_desc = (struct xran_section_desc *)sd_ptr;
                        memcpy(pPrbElem->sec_desc, p_loc_sec_desc, sizeof(struct xran_section_desc));
                    }
                }
            }
        }
...
xranLibWraper::xranLibWraper()
{
    ...
    // From xran_fh_config of xran_fh_o_du.h
    uint8_t enableCP_              = 1;
	...
	/* commenting below to get the ru mac from conf file */
    ...
    m_global_cfg = read_json_from_file_wrap(XRAN_UT_CFG_FILENAME);
    ...
    /* MP_2022: fix because of new version of XRAN library (master against bronze version used by VVDN) */
    memset(&m_xranConf, 0, sizeof(xran_fh_config));
    ...
    m_xranConf.enableCP           = enableCP_;
    m_xranConf.prachEnable        = prachEnable_;
    m_xranConf.debugStop          = debugStop_;
    m_xranConf.debugStopCount     = debugStopCount_;
    m_xranConf.DynamicSectionEna  = DynamicSectionEna_;
            ...
            void *fh_rx_callback, void *fh_rx_prach_callback, void *fh_srs_callback)
{
    ...
    if(m_nInstanceHandle[0] != NULL) 
    {
        for(i = 0; i < nSectorNum; i++) 
        {
            ...
            /* Fix MP_2022: according to spec, change pDstBuffer as second buffer list parameter into pDstBufferDecomp -> At the moment same buffer list */
            xran_5g_prach_req(m_nInstanceHandle[0][i], pFthRxRachBuffer[i], pFthRxRachBuffer[i],
        ...
        {
            /* Fix MP_2022: according to spec, change pDstBuffer as second buffer list parameter into pDstBufferDecomp -> at the moment same buffer list */
            xran_5g_srs_req(m_nInstanceHandle[0][i], pFthRxSrsBuffer[i], pFthRxSrsBuffer[i],
