...
int trx_oran_ctlrecv(openair0_device *device, void *msg, ssize_t msg_len)
{
    ...
    if (s->last_msg == RAU_tick && s->capabilities_sent == 0) 
    {
      ...
      /* For now they are hard coded - try to retrieve the params from openair device */
      ...
      /* MP_2022: BE CAREFUL; Overflow because cap->N_RB_DL & cap->N_RB_UL are uint8_t, so max value is 255 */
      cap->N_RB_DL[0]                       = 100;
      cap->N_RB_UL[0]                       = 100; /* Value max described into defs_RU.h */
