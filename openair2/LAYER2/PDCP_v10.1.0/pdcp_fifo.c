    ...
    if (rb_id == 10) { //hardcoded for PC5-Signaling
      ...
      //ret = write(nas_sock_fd[ctxt_pP->module_id], &(sdu_p->data[sizeof(pdcp_data_ind_header_t)]),sizeToWrite );
       ...
       //LOG_I(PDCP,"[PDCP_FIFOS] ret %d TRIED TO PUSH DATA TO rb_id %d handle %d sizeToWrite %d\n",ret,rb_id,nas_sock_fd[ctxt_pP->module_id],sizeToWrite);
		...
		#endif
#if LATSEQ
      LATSEQ_P("U pdcp.out.nas--ip", "len%d:drb%d.rnti%d.fm%d:sock%d.l2id%d", sizeToWrite, rb_id, ctxt_pP->rnti, ctxt_pP->frame, nas_sock_fd[0], ((pdcp_data_ind_header_t *)(sdu_p->data))->destinationL2Id);
#endif
    	ret = write(nas_sock_fd[0], &(sdu_p->data[sizeof(pdcp_data_ind_header_t)]), sizeToWrite);

    } else if (PDCP_USE_NETLINK) {
#if LATSEQ
      LATSEQ_P("U pdcp.out.nl--ip", "len%d:drb%d.rnti%d.fm%d:sock%d.l2id%d", sizeToWrite, rb_id, ctxt_pP->rnti, ctxt_pP->frame, nas_sock_fd[0], ((pdcp_data_ind_header_t *)(sdu_p->data))->destinationL2Id);
#endif
  ...
  do {
    ...
    if (h_rc == HASH_TABLE_OK) {
      ...
      LOG_D(PDCP, "[FRAME %5u][UE][IP][INSTANCE %u][RB %ld][--- PDCP_DATA_REQ / %d Bytes --->][PDCP][MOD %u][UE %04x][RB %ld]\n",
            ...
            ctxt.rnti, rab_id);
#if LATSEQ
      LATSEQ_P("D ip--pdcp.in.tun", "len%d:drb%d.rnti%d.fm%d", len, rab_id, ctxt.rnti, ctxt.frame);
#endif
#if defined  ENABLE_PDCP_PAYLOAD_DEBUG
            ...
            if (rab_id != 0) {
              ...
              if (h_rc == HASH_TABLE_OK) {
                ...
                LOG_D(PDCP, "[FRAME %5u][eNB][IP][INSTANCE %u][RB %ld][--- PDCP_DATA_REQ / %d Bytes --->][PDCP][MOD %u]UE %u][RB %ld]\n",
                      ...
                      rab_id);
#if LATSEQ
                LATSEQ_P("D ip--pdcp.in.nl", "len%d:drb%d.rnti%d.fm%d", pdcp_read_header_g.data_size, rab_id, ctxt.rnti, ctxt.frame);
#endif
                pdcp_data_req(&ctxt,
                              SRB_FLAG_NO,
                              rab_id,
