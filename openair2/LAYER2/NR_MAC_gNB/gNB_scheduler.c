                                ...
                                sub_frame_t slotP)
{
  ...
  const int size = *scc->ssbSubcarrierSpacing == 0 ? num_slots << 1 : num_slots;
  const int next_slot = frameP * num_slots + slotP + size - 1;
  nfapi_nr_ul_tti_request_t *future_ul_tti_req = &gNB->UL_tti_req_ahead[CC_idP][next_slot % size];
  future_ul_tti_req->SFN = (next_slot / num_slots) % 1024;
  LOG_D(NR_MAC, "%d.%d UL_tti_req_ahead SFN.slot = %d.%d for index %d \n", frameP, slotP, future_ul_tti_req->SFN, future_ul_tti_req->Slot, next_slot % size);
  ...
  const int current_index = ul_buffer_index(frameP, slotP, *scc->ssbSubcarrierSpacing);
...
void gNB_dlsch_ulsch_scheduler(module_id_t module_idP,
  ...
  for (int CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
    ...
    const int num_slots = nr_slots_per_frame[*scc->ssbSubcarrierSpacing];
    const int size = *scc->ssbSubcarrierSpacing == 0 ? num_slots << 1 : num_slots;
    const int index = frame * num_slots + slot + size - 1;
    uint16_t *vrb_map_UL = cc[CC_id].vrb_map_UL;
    memcpy(&vrb_map_UL[index % size * MAX_BWP_SIZE], &RC.nrmac[module_idP]->ulprbbl, sizeof(uint16_t) * MAX_BWP_SIZE);
