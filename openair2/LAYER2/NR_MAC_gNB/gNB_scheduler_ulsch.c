...
bool nr_fr1_ulsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  const int index = ul_buffer_index(sched_frame, sched_slot, mu);
  uint16_t *vrb_map_UL = &RC.nrmac[module_id]->common_channels[CC_id].vrb_map_UL[index * MAX_BWP_SIZE];
...
void nr_schedule_ulsch(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  UE_iterator( UE_info->list, UE) {
    ...
    const int index = ul_buffer_index(sched_pusch->frame, sched_pusch->slot, current_BWP->scs);
