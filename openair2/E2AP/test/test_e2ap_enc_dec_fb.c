...
void test_subscription_response()
{
  ...
  ric_subscription_response_t sr_begin = {.ric_id = ric_id, .admitted = admitted, .len_admitted = len_admitted, .not_admitted = not_admitted, .len_na = len_not_admitted};
...
void test_setup_request()
{
  ...
  e2_setup_request_t sr_begin = {.id = ge2ni, .ran_func_item = r, .len_rf = len_rf};
...
void test_setup_response()
{
  ...
  rej[0] = (rejected_ran_function_t){.id = 4, .cause = {.present = CAUSE_TRANSPORT, .transport = CAUSE_TRANSPORT_TRANSPORT_RESOURCE_UNAVAILABLE}};
  rej[1] = (rejected_ran_function_t){.id = 5, .cause = {.present = CAUSE_PROTOCOL, .protocol = CAUSE_PROTOCOL_SEMANTIC_ERROR}};

  e2_setup_response_t sr_begin = {.id = ric_id, .accepted = acc, .len_acc = len_acc, .rejected = rej, .len_rej = len_rej};
