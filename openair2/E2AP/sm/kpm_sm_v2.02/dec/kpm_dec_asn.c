...
#include "E2SM_KPMv2_MeasurementType.h"
...
static void cp_label_info_item_from_asn(adapter_LabelInfoItem_t *dst, E2SM_KPMv2_LabelInfoItem_t const *src);
...
kpm_event_trigger_t kpm_dec_event_trigger_asn(size_t len, uint8_t const ev_tr[len])
{
  ...
  const asn_dec_rval_t rval = asn_decode(NULL, syntax, &asn_DEF_E2SM_KPMv2_E2SM_KPM_EventTriggerDefinition, (void **)&pdu, ev_tr, len);
  assert(rval.code == RC_OK && "Are you sending data in ATS_ALIGNED_BASIC_PER syntax?");

  kpm_event_trigger_t ret = {0};

  assert(pdu->eventDefinition_formats.present == E2SM_KPMv2_E2SM_KPM_EventTriggerDefinition__eventDefinition_formats_PR_eventDefinition_Format1 && "Invalid Choice");

  ret.ms = pdu->eventDefinition_formats.choice.eventDefinition_Format1->reportingPeriod;

  ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_E2SM_KPMv2_E2SM_KPM_EventTriggerDefinition, pdu);
  free(pdu);
...
kpm_action_def_t kpm_dec_action_def_asn(size_t len, uint8_t const action_def[len]) 
{
  ...
  const enum asn_transfer_syntax syntax = ATS_ALIGNED_BASIC_PER;
  const asn_dec_rval_t rval = asn_decode(NULL, syntax, &asn_DEF_E2SM_KPMv2_E2SM_KPM_ActionDefinition, (void **)&pdu, action_def, len);
  assert(rval.code == RC_OK && "Are you sending data in ATS_ALIGNED_BASIC_PER syntax?");
  ...
  {
    case E2SM_KPMv2_E2SM_KPM_ActionDefinition__actionDefinition_formats_PR_NOTHING:
    ...
    case E2SM_KPMv2_E2SM_KPM_ActionDefinition__actionDefinition_formats_PR_actionDefinition_Format1:;
      ...
      { 
        ...
        {
          case E2SM_KPMv2_MeasurementType_PR_NOTHING: // manages backward compatibility
            continue;
          case E2SM_KPMv2_MeasurementType_PR_measName:
            ...
            break;
          case E2SM_KPMv2_MeasurementType_PR_measID:
  ...
  ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_E2SM_KPMv2_E2SM_KPM_ActionDefinition, pdu);
  free(pdu);
  return ret;
...
kpm_ind_hdr_t kpm_dec_ind_hdr_asn(size_t len, uint8_t const ind_hdr[len])
{
  ...
  const enum asn_transfer_syntax syntax = ATS_ALIGNED_BASIC_PER;
  const asn_dec_rval_t rval = asn_decode(NULL, syntax, &asn_DEF_E2SM_KPMv2_E2SM_KPM_IndicationHeader, (void **)&pdu, ind_hdr, len);
  assert(rval.code == RC_OK && "Are you sending data in ATS_ALIGNED_BASIC_PER syntax?");

  if (pdu->indicationHeader_formats.present == E2SM_KPMv2_E2SM_KPM_IndicationHeader__indicationHeader_formats_PR_indicationHeader_Format1) {
    E2SM_KPMv2_E2SM_KPM_IndicationHeader_Format1_t *hdr = pdu->indicationHeader_formats.choice.indicationHeader_Format1;
  ...
  ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_E2SM_KPMv2_E2SM_KPM_IndicationHeader, pdu);
  free(pdu);
...
kpm_ind_msg_t kpm_dec_ind_msg_asn(size_t len, uint8_t const ind_msg[len])
{
  ...
  const enum asn_transfer_syntax syntax = ATS_ALIGNED_BASIC_PER;
  const asn_dec_rval_t rval = asn_decode(NULL, syntax, &asn_DEF_E2SM_KPMv2_E2SM_KPM_IndicationMessage, (void **)&pdu, ind_msg, len);
  assert(rval.code == RC_OK && "Are you sending data in ATS_ALIGNED_BASIC_PER syntax?");

  if (pdu->indicationMessage_formats.present == E2SM_KPMv2_E2SM_KPM_IndicationMessage__indicationMessage_formats_PR_indicationMessage_Format1) {
    ...
    {
      E2SM_KPMv2_MeasurementDataItem_t *item = msg->measData.list.array[i];
      ...
      for (size_t j = 0; j < (size_t) item->measRecord.list.count; j++){
        switch (item->measRecord.list.array[j]->present){
          case E2SM_KPMv2_MeasurementRecordItem_PR_noValue:
            rec[j].type = MeasRecord_noval;
            break;

          case E2SM_KPMv2_MeasurementRecordItem_PR_integer:
            rec[j].type = MeasRecord_int;
            rec[j].int_val = item->measRecord.list.array[j]->choice.integer;
            break;

          case E2SM_KPMv2_MeasurementRecordItem_PR_real:
    ...
    if (msg->measInfoList != NULL)
    {
      ...
      {
        E2SM_KPMv2_MeasurementInfoItem_t *mInfo = msg->measInfoList->list.array[i];
  ...
  ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_E2SM_KPMv2_E2SM_KPM_IndicationMessage, pdu);
  free(pdu); 
...
kpm_func_def_t kpm_dec_func_def_asn(size_t len, uint8_t const func_def[len])
{
  ...
  const enum asn_transfer_syntax syntax = ATS_ALIGNED_BASIC_PER;
  const asn_dec_rval_t rval = asn_decode(NULL, syntax, &asn_DEF_E2SM_KPMv2_E2SM_KPM_RANfunction_Description, (void **)&pdu, func_def, len);
  assert(rval.code == RC_OK && "Are you sending data in ATS_ALIGNED_BASIC_PER syntax?");
  ...
  ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_E2SM_KPMv2_E2SM_KPM_RANfunction_Description, pdu);
  free(pdu); 
