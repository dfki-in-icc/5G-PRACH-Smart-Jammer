...
uint8_t do_RRCSetupRequest(uint8_t Mod_id, uint8_t *buffer, size_t buffer_size, uint8_t *rv) {
  ...
  if (0) { //if (NR_UE_rrc_inst[Mod_id].paging_flag == 1) { // unclear what the UE instance does in the gNB RRC
    ...
    for (int i = 0; i < 5; i++) {
      rrcSetupRequest->rrcSetupRequest.ue_Identity.choice.ng_5G_S_TMSI_Part1.buf[4-i] = (sTMSI>>(i*8))&0xff;
    }
    ...
    rrcSetupRequest->rrcSetupRequest.establishmentCause = NR_EstablishmentCause_mo_Signalling;
  } else {
    rrcSetupRequest->rrcSetupRequest.ue_Identity.present = NR_InitialUE_Identity_PR_ng_5G_S_TMSI_Part1;
