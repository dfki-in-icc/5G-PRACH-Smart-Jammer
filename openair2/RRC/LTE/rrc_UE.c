...
void rrc_ue_update_radioResourceConfigDedicated(LTE_RadioResourceConfigDedicated_t *radioResourceConfigDedicated,
                                                const protocol_ctxt_t *const ctxt_pP,
                                                uint8_t eNB_index)
{
...
//-----------------------------------------------------------------------------
{
  ...
  if (securityModeCommand->criticalExtensions.present == LTE_SecurityModeCommand__criticalExtensions_PR_c1)
  {
    ...
    LOG_W(RRC, "securityModeCommand->criticalExtensions.present (%d) != SecurityModeCommand__criticalExtensions_PR_c1\n",
...
//-----------------------------------------------------------------------------
{
  ...
  {
    if (*UECapabilityEnquiry->criticalExtensions.choice.c1.choice.ueCapabilityEnquiry_r8.ue_CapabilityRequest.list.array[i] == LTE_RAT_Type_eutra)
    {
      ASN_SEQUENCE_ADD(
...
//-----------------------------------------------------------------------------
void rrc_ue_process_rrcConnectionReconfiguration(
    const protocol_ctxt_t *const ctxt_pP,
    ...
    if (rrcConnectionReconfiguration->criticalExtensions.choice.c1.present ==
      ...
      */
...
#if ENABLE_RAL
      {
        ...
        if (connection_reestablishment_ind.num_srb > 2)
        { // fixme: only 2 srbs can exist, adjust the value
...
//-----------------------------------------------------------------------------
void rrc_ue_process_mobilityControlInfo(
    const protocol_ctxt_t *const ctxt_pP,
    const uint8_t eNB_index,
  ...
  if (dl_dcch_msg->message.present == LTE_DL_DCCH_MessageType_PR_c1)
  {
    if (UE_rrc_inst[ctxt_pP->module_id].Info[eNB_indexP].State >= RRC_CONNECTED)
    {
      ...
      case LTE_DL_DCCH_MessageType__c1_PR_rrcConnectionReconfiguration:
        ...
        {
          /* 36.331, 5.3.5.4 Reception of an RRCConnectionReconfiguration including the mobilityControlInfo by the UE (handover)*/
          if (UE_rrc_inst[ctxt_pP->module_id].HandoverInfoUe.targetCellId != dl_dcch_msg->message.choice.c1.choice.rrcConnectionReconfiguration.criticalExtensions.choice.c1.choice.rrcConnectionReconfiguration_r8.mobilityControlInfo->targetPhysCellId)
          {
            LOG_W(RRC,
          ...
          else if ((target_eNB_index = get_adjacent_cell_mod_id(UE_rrc_inst[ctxt_pP->module_id].HandoverInfoUe.targetCellId)) == 0xFF)
          {
            LOG_W(RRC,
      ...
      case LTE_DL_DCCH_MessageType__c1_PR_rrcConnectionRelease:
        ...
        if ((dl_dcch_msg->message.choice.c1.choice.rrcConnectionRelease.criticalExtensions.present == LTE_RRCConnectionRelease__criticalExtensions_PR_c1) && (dl_dcch_msg->message.choice.c1.choice.rrcConnectionRelease.criticalExtensions.choice.c1.present == LTE_RRCConnectionRelease__criticalExtensions__c1_PR_rrcConnectionRelease_r8))
        {
...
//-----------------------------------------------------------------------------
{
  ...
  if (NB_UE_INST > 0)
  {
    ...
    for (module_id = 0; module_id < NB_UE_INST; module_id++)
    {
      ...
      printf("Inside sidelink active, after init_SL_preconfig\n");

      rrc_mac_config_req_ue(module_id, CC_id, 0, //eNB_index =0
                              (LTE_RadioResourceConfigCommonSIB_t *)NULL,
                              (struct LTE_PhysicalConfigDedicated *)NULL,
                              (LTE_SCellToAddMod_r10_t *)NULL,
                              (LTE_MeasObjectToAddMod_t **)NULL,
                              ...
                              (const uint32_t *)NULL,
                               ...
                               1025,                                          // indicates that there is no  update in the frame number
                               11,                                            // /indicates that there is no update in the subframe number
                               NULL);   
    }
                                              ...
                                              SL_TRIGGER_t mode)
{
  ...
  if (((UE_rrc_inst[ctxt_pP->module_id].Info[eNB_index].SIStatus & 8192) > 0) && (destinationInfoList != NULL))
  ...
  if (((UE_rrc_inst[ctxt_pP->module_id].Info[eNB_index].SIStatus & 16384) > 0) && (discTxResourceReq != NULL))
...
//-----------------------------------------------------------------------------
{
  ...
  if (sib18 != NULL)
  {
    if (sib18->commConfig_r12 != NULL)
    {
      ...
      //TODO
      if (sib18->commConfig_r12->commTxPoolNormalCommon_r12 != NULL)
  ...
  while (1)
  {
    ...
    switch (sl_ctrl_msg_recv->type) {
      ...
      *logicalchannelgroup_drb = 1;
      ...
      printf("Before rrc_pdcp_config_asn1_req\n");
      rrc_pdcp_config_asn1_req(&ctxt,
                               (LTE_SRB_ToAddModList_t *)NULL,
                               UE->DRB_configList,
                               (LTE_DRB_ToReleaseList_t *)NULL,
      ...
      printf("After rrc_pdcp_config_asn1_req\n");
      printf("Before rrc_rlc_config_asn1_req\n");
      rrc_rlc_config_asn1_req(&ctxt,
                              (LTE_SRB_ToAddModList_t *)NULL,
                              UE->DRB_configList,
                              (LTE_DRB_ToReleaseList_t *)NULL,
    ...
    case GROUP_COMMUNICATION_RELEASE_REQ:
      ...
      rrc_mac_config_req_ue(module_id, 0, 0, //eNB_index =0
                            ...
                            NULL, 0,
                            (LTE_MBSFN_AreaInfoList_r9_t *)NULL,
                            (LTE_PMCH_InfoList_r9_t *)NULL, CONFIG_ACTION_REMOVE,
                            &sourceL2Id,
                            &destinationL2Id,
                            0,
                            (struct LTE_NonMBSFN_SubframeConfig_r14 *)NULL,
    ...
    case DIRECT_COMMUNICATION_ESTABLISH_REQ:
      ...
      *logicalchannelgroup_drb = 1;
      ...
      rrc_rlc_config_asn1_req(&ctxt,
                              ...
                              UE->DRB_configList,
                              (LTE_DRB_ToReleaseList_t *)NULL,
                              (LTE_PMCH_InfoList_r9_t *)NULL, sourceL2Id, destinationL2Id);
                  ...
                  const uint8_t Sdu_len)
{
  ...
  rrc_mac_config_req_ue(ctxt_pP->module_id, 0, 0, //eNB_index =0
                              (LTE_RadioResourceConfigCommonSIB_t *)NULL,
                              (struct LTE_PhysicalConfigDedicated *)NULL,
                              (LTE_SCellToAddMod_r10_t *)NULL,
                              (LTE_MeasObjectToAddMod_t **)NULL,
                              ...
                              &UE_rrc_inst[ctxt_pP->module_id].mib_sl[0]->sl_Bandwidth_r12    );
...
//-----------------------------------------------------------------------------
{
  ...
  if (UE_rrc_inst[ctxt_pP->module_id].Info[enb_indexP].T300_active == 1)
  {
    ...
    if (UE_rrc_inst[ctxt_pP->module_id].Info[enb_indexP].T300_cnt == T300[UE_rrc_inst[ctxt_pP->module_id].sib2[enb_indexP]->ue_TimersAndConstants.t300])
    {
  ...
  {
    if (UE_rrc_inst[ctxt_pP->module_id].Info[enb_indexP].N310_cnt == N310[UE_rrc_inst[ctxt_pP->module_id].sib2[enb_indexP]->ue_TimersAndConstants.n310])
    {
  ...
  {
    if (UE_rrc_inst[ctxt_pP->module_id].Info[enb_indexP].N311_cnt == N311[UE_rrc_inst[ctxt_pP->module_id].sib2[enb_indexP]->ue_TimersAndConstants.n311])
    {
